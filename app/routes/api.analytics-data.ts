...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const cacheKey = generateCacheKey('analytics-data', shop);

  // If bypassing cache, invalidate it first
  if (bypassCache) {
    cache.delete(cacheKey);
    console.log(`[Analytics Data API] Cache bypassed for ${shop}`);
  }

  // Use cache with 5 minute TTL (300 seconds)
  const cachedData = await withCache(cacheKey, 300, async () => {
    console.log(`[Analytics Data API] Fetching fresh data for ${shop}`);
