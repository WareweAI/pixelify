...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const cacheKey = generateCacheKey('events-data', shop);

  // If bypassing cache, invalidate it first
  if (bypassCache) {
    cache.delete(cacheKey);
    console.log(`[Events Data API] Cache bypassed for ${shop}`);
  }

  // Use cache with 5 minute TTL (300 seconds)
  const cachedData = await withCache(cacheKey, 300, async () => {
    console.log(`[Events Data API] Fetching fresh data for ${shop}`);
